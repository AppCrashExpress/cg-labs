...
std::vector<float> HorseshoeMesh::getData() const
{
    std::vector<float> data;

    data.reserve(27 * polygons_.size());

    for (const auto& p : polygons_) {
        std::vector<float> poly_float = p.getVerticesFloat();
        data.insert(data.end(), poly_float.begin(), poly_float.end());
    }
